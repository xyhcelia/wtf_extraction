...
if(@ARGV==0){
die "Usage:\
perl $0  --ann-wtf_LTR_Tf  input.fa(.fasta) wtfdb\
\"wtfdb\" means database of known wtf sequences for blast;\ 
or\
perl $0  input.fa(.fasta)  wtf_LTR_Tf.feature.txt\  
...
if($ARGV[0] eq "--ann-wtf_LTR_Tf"){
    $inputfa=$ARGV[1];
    $id = basename($inputfa,@suffixlist);
    system("blastn -query $inputfa   -db $ARGV[2] -outfmt 7  -out query-${id}.wtf.out"
);
    ...
    system("blastn -query $inputfa   -db /data/XYH/snapgene_gbk_generator/LTR -task blastn -outfmt 7  -out query-${id}.LTR.out");
    ...
    system("blastn -query $inputfa   -db /data/XYH/snapgene_gbk_generator/Tf_CDS -task blastn -outfmt 7  -out query-${id}.Tf.out");
...
system("blastn -query $inputfa   -db /data/XYH/snapgene_gbk_generator/PomBase_CDS -outfmt 7  -out query-${id}.CDS.out");

######CDS feature file########
my %cdsname;
open CDSTXT,"</data/XYH/snapgene_gbk_generator/Schizosaccharomyces_pombe.ASM294v2.45.gff3.PomBase_CDS.nowtf.noTf.link.txt";
while(<CDSTXT>){
    chomp;
    my @arr=split;
...
if($ARGV[0] eq "--ann-wtf_LTR_Tf"){ 
     ...
     `cat ${id}.CDS.feature.txt $ARGV[1] >$id.feature.txt`;
...
while(<FEATURE>){
   ...
   if($arr[5] eq "wtf" && $arr[4] ne "wtf"){
       `cat /data/XYH/pool_wtf_sequences/gene_structure_annotation/PacBio_1-4_wolf_and_amplicon.wtf.exon.feature.corrected.txt|grep $arr[4] >$arr[4].gene_structure.txt`;          
        if($arr[3] eq "+"){
