...
Other Notice: Before running this script,there should preexist a file containing positiong of wtf loci, "/data/XYH/reference/wtf_closest_CDS.fa" and "/data/XYH/reference/wtf_closest_CDS.txt";
EOF
     exit
fi
...
blastn -query /data/XYH/reference/wtf_closest_CDS.fa -db $id -outfmt 7  -out $id.wtfregions.out
perl $path/bin/blast_1st_hits.pl $id.wtfregions.out $id.wtfregions.1st.out
perl $path/bin/add_feature.pl -i /data/XYH/reference/wtf_closest_CDS.txt -c1 1 -p $id.wtfregions.1st.out -c2 1 -c3 2,9,10 -o wtf_closest_CDS.${id}_contig.txt
perl $path/bin/paste.pl wtf_closest_CDS.${id}_contig.txt ${id}_wtf_region.txt >${id}_unextracted.txt

rm   $id.n* $id.wtfregions.1st.out  wtf_closest_CDS.${id}_contig.txt 

#########wtf region fasta file###########
perl $path/bin/03_extract_fasta_from_ref.pl ../$1 ${id}_wtf_region.txt ${id}_wtf_region.fa
perl $path/bin/wtf_region_reverse.pl /data/XYH/reference/wtf_gene.tandem_link.newname.bed  ${id}_wtf_region.fa  $id ${id}_wtf_region.reverse.txt
perl $path/bin/03_extract_fasta_from_ref.pl ${id}_wtf_region.fa ${id}_wtf_region.reverse.txt ${id}_wtf_region.reverse.fa
...
perl  $path/bin/fasta_length.pl $2 >known.wtf.len.txt
perl  $path/bin/wtfregion_wtf_ends.newname.pl  known.wtf.len.txt  ${id}_wtf_region.reverse.wtf.out ${id}_wtf_region.reverse.wtf.txt  $3 >${id}_wtf_region.reverse.wtf.error

rm ${id}_wtf_region.reverse.wtf.out known.wtf.len.txt

perl $path/bin/03_extract_fasta_from_ref.pl  ${id}_wtf_region.reverse.fa ${id}_wtf_region.reverse.wtf.txt $id.wtf.routine.fa
...
for loci  in ` cat error.list`;do
      ...
      perl $path/bin/wtfregions_snapgene_generator.pl -i $loci-flrvs.fa --ann-wtf_LTR_Tf -db known.wtf
      rm $loci-flrvs.fa
done
...
perl $path/bin/03_extract_fasta_from_ref.pl $id.normal_wtf_region_mask.fa $id.extra_wtf.region.txt $id.extra_wtf.region.fa
...
for seqname  in ` cat $id.extra_wtf.region.list`;do
      ...
      perl $path/bin/wtfregions_snapgene_generator.pl -i $seqname.fa --ann-wtf_LTR_Tf -db known.wtf
      rm $seqname.fa
done
