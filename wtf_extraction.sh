...
perl  $path/bin/wtfregion_wtf_ends.2.pl  known.wtf.len.txt  ${id}_wtf_region.reverse.wtf.out ${id}_wtf_region.reverse.wtf.txt  $3 >${id}_wtf_region.reverse.wtf.error
...
for loci  in ` cat error.list`;do
      ...
      perl $path/bin/wtfregions_snapgene_generator.2.pl --ann-wtf_LTR_Tf $loci-flrvs.fa known.wtf
      rm $loci-flrvs.fa
done
...
for seqname  in ` cat $id.extra_wtf.region.list`;do
      ...
      perl $path/bin/wtfregions_snapgene_generator.2.pl --ann-wtf_LTR_Tf $seqname.fa known.wtf
      rm $seqname.fa
done
